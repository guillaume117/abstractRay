...
tensor_test = torch.randn(2000,3,42,42)
tensor_test = torch.where(tensor_test>1,tensor_test,0).to_sparse()
print(f'tensor_test nnz = {tensor_test._nnz()}')
size = torch.Size([20000, 3, 4, 4])
sparse_tensor = torch.sparse_coo_tensor(indices, values, size,dtype=torch.int32)
...
with torch.no_grad():
    print(torch.sum(output.to_dense()-conv2(tensor_test.to_dense())))
